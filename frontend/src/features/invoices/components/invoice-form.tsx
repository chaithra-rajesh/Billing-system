...
import { PageActions } from '@/components/layout/app-shell';
...
import type { FranchiseContext, FranchiseFull } from '@/features/franchises/api';
...
export interface InvoiceFormProps {
  ...
  cancelHref: () => void;
  onSaveDraft: (values: InvoiceFormValues) => Promise<void> | void;
  onFinalise?: (values: InvoiceFormValues) => Promise<void> | void;
}
...
export function InvoiceForm({
  ...
  initialValues,
  initialCustomer,
  invoiceNoLabel = '— assigned on save —',
  saving,
  showFinaliseButton,
  finaliseLabel = 'Save & finalise',
  cancelHref,
  onSaveDraft,
  onFinalise,
}: InvoiceFormProps) {
  ...
  const customersQuery = useCustomers(franchise.id, {
  ...
  const form = useForm<InvoiceFormValues>({
    defaultValues: initialValues ?? defaultInvoiceFormValues(),
  });
  ...
  // Seed editable percentages from active gst_config when ctx loads, but only
  // if the form currently has zeros (don't clobber pre-filled values from a
  // draft).
  useEffect(() => {
    if (!gst) return;
    if (form.getValues('cgst_percent') === 0)
      form.setValue('cgst_percent', Number(gst.cgst_percent));
    if (form.getValues('sgst_percent') === 0)
      form.setValue('sgst_percent', Number(gst.sgst_percent));
    if (form.getValues('igst_percent') === 0)
      form.setValue('igst_percent', Number(gst.igst_percent));
  }, [gst, form]);

  // Auto-flip tax mode when the picked customer's state differs from franchise.
  useEffect(() => {
    if (!franchise.state_code || !customer?.state_code) return;
    const intra = franchise.state_code === customer.state_code;
    form.setValue('tax_mode', intra ? 'intra' : 'inter', { shouldDirty: false });
  }, [franchise.state_code, customer?.state_code, form]);
  ...
  const handleSaveDraft = () =>
    form.handleSubmit((values) => onSaveDraft(values))();

  const onSubmit = (values: InvoiceFormValues) => {
    if (onFinalise) return onFinalise(values);
  };

  const canFinalise = !!gst && !!bank;

  return (
    <>
      <PageActions>
        <Button variant="outline" type="button" onClick={cancelHref}>
          Cancel
        </Button>
        <Button
          variant="outline"
      ...
      <div className="mx-auto w-full max-w-5xl">
        ...
        >
          ...
          <div className="grid grid-cols-1 border-b-2 border-black md:grid-cols-2">
            <div className="border-b-2 border-black md:border-b-0 md:border-r-2">
              <MetaRow
                label="Invoice No."
                value={<span className="text-muted-foreground">{invoiceNoLabel}</span>}
              />
              <MetaRow
                label="Invoice Date"
                input={
          ...
          <div className="grid grid-cols-1 border-b-2 border-black md:grid-cols-[1fr_340px]">
            <div className="border-b-2 border-black p-2 md:border-b-0 md:border-r-2">
              ...
              <div className="mt-3 flex items-center gap-2 text-[10px] text-muted-foreground">
                ...
                {customer?.state_code && franchise.state_code && (
                  <span className="text-[10px]">
                    auto-detected:{' '}
                    {customer.state_code === franchise.state_code ? 'intra' : 'inter'}
                  </span>
                )}
              </div>
            </div>
        ...
        {showFinaliseButton && (
          <p className="mt-3 text-xs text-muted-foreground print:hidden">
            {!gst && 'No active GST configuration on this franchise. Add one in admin before finalising. '}
            {!bank && 'No active bank details on this franchise. Add one in admin before finalising.'}
          </p>
        )}
      </div>
    </>
